...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author} kimi yasaklayacağını yazmadın! \`.ban @Mavera/ID <sebep>\``)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    ...
    message.channel.send(embed.setDescription(`${user} kullanıcı ${message.author} tarafından **${reason}** sebebiyle yasaklandı.`)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
...
exports.conf = { name: "ban", aliases: [], permLevel: 0 }