...
exports.run = async(client, message, args) => {
    ...
    message.channel.send(embed.setDescription(`${victim}, ${message.author} yanınıza gelmek istiyor. Kabul ediyor musunuz?`)).then(async(msg) => {
        msg.react(ayar.yes)
        msg.react(ayar.no)
        ...
        onay.on("collect", async() => {
            ...
            message.channel.send(embed.setDescription(`${message.author}, ${victim} kullanıcısının yanına **gönderdim.**`)).then(m => m.delete({ timeout: 7000 }))
            msg.delete({ timeout: 7000 })
            message.delete()})

        red.on("collect", async() => {
            await msg.reactions.removeAll()
            message.channel.send(embed.setDescription(`${message.author}, ${victim} kullanıcısı soruyu reddettiği için **gönderemedim.**`)).then(m => m.delete({ timeout: 7000 }))
            msg.delete({ timeout: 7000 })
            message.delete()})})}

exports.conf = { name: "igit", aliases: ['izinligit', 'izinli-git'], permLevel: 0 }