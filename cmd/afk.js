...
exports.run = async(client, message, args) => {
    ...
    let sebep = args.join(' ');
    if (sebep && yasaklar.includes(sebep)) return message.channel.send('Yasaklı kelimelerle AFK olamazsın.').then(x => x.delete({ timeout: 5000 }) && message.delete());
...
exports.conf = { name: "afk", aliases: [], permLevel: 0 }