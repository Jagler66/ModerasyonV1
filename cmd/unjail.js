...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author} kimin yasağını kaldıracağını yazmadın! \`.unjail @Mavera/ID\``)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    ...
    client.channels.cache.get(ayar.jailLog).send(embed.setDescription(`${user} kullanıcısının **karantina** cezası ${message.author} tarafından kaldırıldı.`))}

exports.conf = { name: "unjail", aliases: [], permLevel: 0 }