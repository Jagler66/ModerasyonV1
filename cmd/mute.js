...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author} kime cezalandıracağını yazmadın! \`.mute @Mavera/ID <süre> <sebep>\``)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    if (!args[1]) return message.channel.send(embed.setDescription(`${message.author} chat-mute süresi belirtmedin! \`.mute @Mavera/ID <süre> <sebep>\``)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    let sure = args[1]
        .replace("s", " Saniye")
        .replace("m", " Dakika")
        .replace("h", " Saat")
    ...
    if (durum) return message.channel.send(embed.setDescription(`Kullanıcı zaten muteli.`))

    user.roles.add(ayar.muteRol)
    message.react(ayar.yes)
    message.channel.send(embed.setFooter(`Üyenin toplam ${puan} ceza puanı bulunmakta.`).setDescription(`${user} kullanıcısı ${message.author} tarafından **${reason}** sebebiyle  \`${sure}\` süresince chat-mute cezası verdi.`)).then(x => x.delete({ timeout: 7000 }) && message.delete({ timeout: 6999 }))
...
`))
    ...
    setTimeout(async() => {
        ...
        if (log) log.send(embed.setDescription(`${user} kullanıcısının **chat-mute** ceza süresi bittiği için mutesini açtım.`))
        kdb.delete(`durum.${user.id}.mute`)
    }, ms(args[1]))}

exports.conf = { name: "mute", aliases: ["chatmute", "cmute"], permLevel: 0 }