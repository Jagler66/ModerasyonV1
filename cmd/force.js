...
exports.run = async(client, message, args) => {
    ...
    message.channel.send(embed.setDescription(`<@!${user.id}> (\`${user.tag}\`) kullanıcısı **${sebep}** sebebiyle yasaklandı.`)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
...
exports.conf = { name: "force", aliases: ['forceban', 'force-ban', 'idban', 'ıdban'], permLevel: 0 }